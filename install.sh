...
#           (--windows-keys adds Windows-style shortcuts, opt-in and never part of --full; see hypr/orbital-keys-windows.lua),
...
reconcile_hypr_requires() {
  ...
  for mod in orbital orbital-gaps orbital-bindings orbital-gestures orbital-keys-windows orbital-keyboard; do
...
uninstall() {
  ...
  # Take the require() lines out first: Hyprland reloads when a file it loads disappears, and a
  # require without its file is a hard config error. Only what this installer put there is removed:
  # symlinks and a keyboard file you wrote yourself stay, and so do the require lines that go with them.
  [[ -f $HYPR/hyprland.lua ]] && run sed -i --follow-symlinks '/-- orbital$/d' "$HYPR/hyprland.lua"
  local f
  for f in orbital orbital-gaps orbital-bindings orbital-gestures orbital-keys-windows; do
    [[ -L $HYPR/$f.lua ]] || run rm -f "$HYPR/$f.lua"
  done
  if [[ ! -L $HYPR/orbital-keyboard.lua ]] && grep -qs 'generated by orbital install.sh' "$HYPR/orbital-keyboard.lua"; then
    run rm -f "$HYPR/orbital-keyboard.lua"
  elif [[ -e $HYPR/orbital-keyboard.lua ]]; then
...
checks() {
  ...
  for f in hypr/orbital.lua hypr/orbital-keys-windows.lua systemd/orbital.conf .omarchy-theme.yml; do
...
# Windows-style shortcuts: only with --windows-keys (they replace some Omarchy defaults). Left alone when your own
# config already binds the Windows-style clipboard/search keys, so nothing ends up bound twice.
windows_keys_setup() {
  (( WINDOWS_KEYS )) || return 0
  if user_hypr_has '"SUPER \+ V"' || user_hypr_has '"SUPER \+ SHIFT \+ S".*(Snip|screenshot)'; then
    echo "    Windows keys: your Hyprland config already has Windows-style shortcuts; not adding a second set."; return 0
  fi
  say "Windows-style shortcuts (hypr/orbital-keys-windows.lua)"
  run cp "$REPO/hypr/orbital-keys-windows.lua" "$HYPR/orbital-keys-windows.lua" || { problem "could not install $HYPR/orbital-keys-windows.lua"; return 1; }
  if [[ -f $HYPR/hyprland.lua ]] && (( ! DRY )) && ! grep -qF 'hypr.orbital-keys-windows' "$HYPR/hyprland.lua" \
    && ! printf 'require("hypr.orbital-keys-windows") -- orbital\n' >> "$HYPR/hyprland.lua"; then
    problem "could not add the Windows keys require to $HYPR/hyprland.lua"; return 1
  fi
}
