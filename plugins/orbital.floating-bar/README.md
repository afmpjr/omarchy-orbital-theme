> **Fork notice:** this is Orbital's copy of `charlieras262/floating-bar` (MIT, Copyright (c) 2026 Charlieras262; see `LICENSE`),
> renamed to `orbital.floating-bar` so it can coexist with the original. Only change: the `floatGapScale` option in `Bar.qml`.
...
```bash
omarchy bar use orbital.floating-bar   # switch to this bar
...
```bash
omarchy plugin remove orbital.floating-bar
```
