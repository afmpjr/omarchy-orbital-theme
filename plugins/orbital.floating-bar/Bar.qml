...
Item {
  ...
  // Local patch: multiplies the auto-detected (Hyprland gaps_out) margin, so
  // the bar can sit closer to the edges than windows do (0.5 = half) while
  // still following the window-gaps toggle (0 stays 0). shell.json `bar`
  // entry: "floatGapScale" (0..2, default 1). Ignored when floatGap is set.
  property real configuredFloatGapScale: 1
  readonly property real floatGap: configuredFloatGap >= 0 ? configuredFloatGap : autoDetectedGap * configuredFloatGapScale
  ...
  function applyBarConfig() {
    ...
    configuredFloatGapScale = typeof config.floatGapScale === "number"
      ? Math.max(0, Math.min(2, config.floatGapScale)) : 1
  ...
  component BarPanel: PanelWindow {
    ...
    Rectangle {
      ...
      //
      // ORBITAL PATCH: this Omarchy install (4.0.3) doesn't define
      // Style.shellOpacity at all (no Omablur-family plugin installed) --
      // confirmed via `grep -rn shellOpacity /usr/share/omarchy/shell`,
      // zero matches. Reading it back then is `undefined`, which QML logs
      // ("Unable to assign [undefined] to double") and silently treats as
      // fully opaque, so the bar rendered solid instead of the glass look
      // the Orbital theme's shell.toml already specifies
      // ([bar].background-alpha). Falling back to that same value here
      // keeps the intent (translucent glass) working without Omablur.
      // Safe to drop this fallback once Style.shellOpacity ships upstream
      // or Omablur gets installed -- `typeof` guards against either.
      opacity: root.transparent ? 1 : (typeof Style.shellOpacity !== "undefined" ? Style.shellOpacity : 0.72)
