...
// Description per configured layout, from `xkbcli list`: "layout|variant" -> "English (US)".
function layoutDescriptions(text) {
  var out = {}, layout = null, variant = ""
  String(text || "").split("\n").forEach(function (line) {
    var f = line.match(/^- layout: '?([^']*)'?$/)
    if (f) { layout = f[1]; variant = ""; return }
    if (layout === null) return
    var v = line.match(/^  variant: '?([^']*)'?$/)
    if (v) { variant = v[1]; return }
    var d = line.match(/^  description: (.*)$/)
    if (d) { out[layout + "|" + variant] = d[1]; layout = null }
  })
  return out
}
